...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/nikhil-amin/together-plan.git # Replace with your repo URL
    cd together-plan
    ```
...
3.  **Set up Firebase:**
    ...
    *   **Enable Firebase Authentication:**
        ...
        *   Add your app's domain to the "Authorized domains" list (e.g., `localhost` for development, and `nikhil-amin.github.io` for GitHub Pages deployment).
...
1.  **Repository Setup:**
    ...
    *   Go to your repository settings on GitHub (`https://github.com/nikhil-amin/together-plan/settings`).
...
2.  **`basePath` Configuration (Crucial for GitHub Pages):**
    If your GitHub Pages site is served from a subpath (e.g., `https://nikhil-amin.github.io/together-plan/`), you **MUST** configure the `basePath` in your `next.config.ts`.
    Open `next.config.ts` and ensure the `basePath` property is set correctly:
    ```javascript
    // next.config.ts
    const nextConfig = {
      output: 'export',
      basePath: '/together-plan', // <<-- IMPORTANT: Change this to your repository name!
      // ... other configs
    };
    ```
    For example, if your repository is named `together-plan`, set `basePath: '/together-plan'`. If you deploy to `nikhil-amin.github.io` directly (by naming your repository `nikhil-amin.github.io`), then `basePath` is not needed (set it to `''` or remove it).

3.  **Deploying the `out` Folder:**
    ...
    *   **Automated Deployment (Recommended - using GitHub Actions):**
        ...
        jobs:
          build-and-deploy:
            ...
            env: # Pass your Firebase public env vars here
              # IMPORTANT: Set basePath dynamically if possible, or ensure it's correct in next.config.ts
              # For example, if your repo is 'together-plan', basePath should be '/together-plan'

              # This example assumes basePath is correctly set in next.config.ts
        ...
        *   Ensure your `next.config.ts` has the correct `basePath` set if deploying to a subfolder like `nikhil-amin.github.io/together-plan/`.

4.  **Accessing Your Site:**
    Once deployed, your site should be accessible at `https://nikhil-amin.github.io/together-plan/` (if using a subpath) or `https://nikhil-amin.github.io/` (if your repository is named `nikhil-amin.github.io`). Remember to add this URL (including the subpath if applicable) to your Firebase Authentication "Authorized domains".
